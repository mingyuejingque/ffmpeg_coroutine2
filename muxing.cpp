...
static const int g_avio_buffer_len = 32 * 1024;
...
void fill_data_to_ffmpeg(ff_context* ctx, uint8_t* data, int size) {
    if (data && size) {
        int will_write = FFMIN(size, av_fifo_space(ctx->fifo));
        av_fifo_generic_write(ctx->fifo, data, will_write, nullptr);
        printf("%s write %d bytes to fifo\n", __func__, will_write);
    }
}
...
static bool open_output_file(struct ff_context* ctx) {
    int ret = 0;
    if (ctx->has_output_file && ctx->output_file_name) {
        ...
        for (int i = 0; i < ctx->ifmt_ctx->nb_streams; ++i) {
            ...
            if (in_codepar->codec_type != AVMEDIA_TYPE_AUDIO
                && in_codepar->codec_type != AVMEDIA_TYPE_VIDEO) {
                continue;
            }
...
static bool handle_frame_data(ff_context* ctx) {
    ...
    if (ctx->has_probe_input) {
        ...
        while (1) {
            ...
            if (ret < 0) {
                printf("%s av_read_frame ret: %d\n",
                        __func__, ret);
                break;
            }
            in_stream = ctx->ifmt_ctx->streams[ctx->pkt.stream_index];
            out_stream = ctx->ofmt_ctx->streams[ctx->pkt.stream_index];
            log_packet(ctx->ifmt_ctx, &ctx->pkt, "in");
            
            static double frame_rate = av_q2d(in_stream->avg_frame_rate);
            /*copy packet.*/
